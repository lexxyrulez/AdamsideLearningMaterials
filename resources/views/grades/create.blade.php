...
<div style="width: 100%; min-height: 100vh; background: #f0f4f8; padding: 20px; box-sizing: border-box; font-family: 'Roboto', sans-serif; display: flex; animation: fadeIn 1s ease;">
    ...
    <div style="flex: 1; padding: 20px; animation: fadeInUp 1s ease;">
        <h2 style="color: #03a9fc; font-size: 2.5em; margin-bottom: 20px; animation: bounceIn 0.8s;">Add New Grade</h2>
        <form method="POST" action="{{ route('grades.store') }}" style="max-width: 400px; animation: slideUp 0.8s;">
            @csrf
            <input type="text" name="name" placeholder="Grade Name" required style="width: 100%; padding: 12px; border: 2px solid #03a9fc; border-radius: 8px; margin-bottom: 20px; font-size: 1em; transition: border-color 0.3s;">
