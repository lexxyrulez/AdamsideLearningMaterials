...
<div style="width: 100%; min-height: 100vh; background: #f0f4f8; padding: 20px; box-sizing: border-box; font-family: 'Roboto', sans-serif; display: flex; justify-content: center; align-items: center; animation: fadeIn 1s ease;">
    <div style="max-width: 500px; width: 100%; background: #fff; padding: 30px; border-radius: 15px; box-shadow: 0 8px 16px rgba(0,0,0,0.1); animation: slideUp 0.8s ease;">
        <h2 style="color: #03a9fc; font-size: 2.5em; text-align: center; margin-bottom: 20px; animation: pulse 1.5s infinite;">Admin Registration</h2>
        @if ($errors->any())
        ...
        @endif
        <form method="POST" action="{{ route('register.admin.post') }}" style="display: flex; flex-direction: column; gap: 20px; animation: fadeInUp 1s ease;">
            @csrf
            ...
            <input type="password" name="password" placeholder="Password" required style="padding: 12px; border: 2px solid #03a9fc; border-radius: 8px; font-size: 1em; transition: border-color 0.3s, box-shadow 0.3s; animation: slideIn 0.9s;">
            <input type="password" name="password_confirmation" placeholder="Confirm Password" required style="padding: 12px; border: 2px solid #03a9fc; border-radius: 8px; font-size: 1em; transition: border-color 0.3s, box-shadow 0.3s; animation: slideIn 1s;">
